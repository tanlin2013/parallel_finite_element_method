...
!C +--------+
!C===
      open (35, file=meshfil, form='formatted')
        write (35,'(10i10)') my_rank
        write (35,'(10i10)') NEIBPETOT
        write (35,'(10i10)') (NEIBPE(ib), ib= 1, NEIBPETOT)

        write (35,'(10i10)') NP, N
     
        do i= 1, NP
          write (35,'(2i10,3(1pe16.6))') pNODE_ID(i,1), pNODE_ID(i,2),
     &                                   (XYZ(k,i),k=1,3)
        enddo

        write (35,'(10i10)') ICELTOT, ICELTOT_INT
        write (35,'(10i10)') (N8, ii= 1, ICELTOT)
        do icel= 1, ICELTOT
          write (35,'(i10,2i5,8i10)') 
     &                          pELEM_ID(icel,1), pELEM_ID(icel,2), N1, 
     &                         (ICELNOD(icel,k), k= 1,8)
        enddo
        write (35,'(10i10)') (INT_ELM_LIST(k),k= 1, ICELTOT_INT)

        if (NEIBPETOT.ne.0) then
          write (35,'(10i10)')(NOD_STACK_IMPORT(neib), neib=1,NEIBPETOT)
          do k= 1, NOD_STACK_IMPORT(NEIBPETOT)
            write (35,'(10i10)') NOD_IMPORT(k)
          enddo
        endif

        if (NEIBPETOT.ne.0) then
          write (35,'(10i10)')(NOD_STACK_EXPORT(neib), neib=1,NEIBPETOT)
          do k= 1, NOD_STACK_EXPORT(NEIBPETOT)
            write (35,'(10i10)') NOD_EXPORT(k)
          enddo
        endif

        write (35,'(10i10)') N4
        write (35,'(10i10)') (BC_STACK(k), k=1,N4)
        do ig= 1, N4
          write (35,'(a80)') BC_NAME(ig)
          iS= BC_STACK(ig-1)+1
          iE= BC_STACK(ig)
          if (BC_STACK(ig)-BC_STACK(ig-1).ne.0) then
            write (35,'(10i10)') (BC_NOD(kk), kk= iS, iE)
          endif
        enddo
      close (35)
!C===
